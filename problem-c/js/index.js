...
function extractOpponent (game1) {
  if (game1["home"] == "UW") {
    return game1["opponent"];
  } else if (game1["opponent"] == "UW") {
    return game1["home"];
  }
}
...
const opponent1 = huskyGames2016.map(extractOpponent);
console.log(opponent1);

//Define a function `huskiesLost()` that takes in a "game" object and returns
//whether or not UW lost.
function huskiesLost (game2) {
  if (game2["home"] == "UW") {
    return game2["home_score"] < game2["opponent_score"];
  } else if (game2["opponent"] == "UW") {
    return game2["opponent_score"] < game2["home_score"];
  }
}
...
let opponentUW = lostGames.map(extractOpponent);
console.log(opponentUW);
...
lostGames.forEach(function(game) {
  if (game.home === "UW") {
    ...
    console.log(`UW at ${game.home}, ${game.opponent_score} to ${game.home_score}`);
  }
});
...
function mostYardsPassing(gameA, gameB) {
  if (!gameA.passing_yards) {
    return gameB;
  } else if (gameA.passing_yards > gameB.passing_yards) {
    return gameA;
  } else {
    return gameB;
  }
