...
let result = motto.indexOf("cool") !== -1;
console.log(result);
...
motto =motto.replaceAll("iSchool", "Information School");
console.log(motto);
...
let mottoLengthNew = motto.length;
let ratio = (mottoLengthNew/ mottoLength) * 100;
...
let thatIndex = numbers.indexOf(4);
numbers[thatIndex] = 4.2;
console.log(numbers);
...
if (numbers.length%2==0) {
    let indexEven1 = numbers.length/2;
    let indexEven2 = numbers.length/2 - 1;
    let medianEven = Math.floor((numbers[indexEven1] + numbers[indexEven2])/2);
    console.log(medianEven);
} else {
    let indexOdd = Math.floor(numbers.length / 2);
    let medianOdd = numbers[indexOdd];
...
let lastNum = numbers[lengthNum - 1];
let circle = {
    cx: 34,
    cy: 43,
    radius: lastNum
